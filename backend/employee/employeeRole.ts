...
export abstract class EmployeeRole {
  ...
  async updateClaimBankAccountDetails(
    claim: Claim,
    updateClaimBankAccountDetails: {
    ...
    if (claim.getEmployeeId() !== this.userId) {
      console.error(
        "Claim does not belong to this employee",
        claim.getId(),
        claim.getEmployeeId(),
        this.userId
      );
    ...
    const updateClaim = await db.updateClaimBankDetails(
      claim.getId(),
      updateClaimBankAccountDetails
    );
    if (!updateClaim) {
      console.error(
        "Failed to update claim bank account details",
        claim.getId()
      );
      return null;
    }

    if (updateClaimBankAccountDetails.accountName)
      claim.setAccountName(updateClaimBankAccountDetails.accountName);
    if (updateClaimBankAccountDetails.accountNumber)
      claim.setAccountNumber(updateClaimBankAccountDetails.accountNumber);
    if (updateClaimBankAccountDetails.sortCode)
      claim.setSortCode(updateClaimBankAccountDetails.sortCode);
  ...
  async submitClaim(claim: Claim): Promise<Claim | null> {
    ...
    if (claim.getAttemptCount() >= Claim.MAX_ATTEMPT_COUNT) {
      console.error(
        "Claim has reached maximum attempt count",
        claim.getId(),
    ...
    const result2 = db.updateClaimAttemptCount(
      claim.getId(),
      claim.getAttemptCount() + 1
    );
  ...
  async appealClaim(claim: Claim): Promise<Claim | null> {
    if (claim.getStatus() !== ClaimStatus.REJECTED) {
      console.error(
        "Claim is not in rejected status",
        claim.getId(),
        claim.getStatus()
      );
      return null;
